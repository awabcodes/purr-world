...
export class FavoriteService {
  ...
  list(req?: { sub_id?: string, page?: number, limit?: number }): Observable<HttpResponse<Favorite[]>> {
    const options = createRequestOption(req);
    return this.http.get<Favorite[]>(this.resourceUrl, { params: options, observe: 'response' });
  }

  create(favorite: Favorite): Observable<HttpResponse<{ message: string, id: number }>> {
    return this.http.post<{ message: string, id: number }>(this.resourceUrl, favorite, { observe: 'response' });
  }
